...
class PrinterController {
    ...
    static async create(req, res) {
        try {
            ...
            const result = await dbRun(`INSERT INTO printer_configs (destination, type, name, usbPort, networkIp, networkPort, isActive)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [destination, type, name, usbPort || null, networkIp || null, networkPort || null, isActive ? 1 : 0]);
    ...
    static async update(req, res) {
        try {
            ...
            await dbRun(`UPDATE printer_configs 
         SET destination = ?, type = ?, name = ?, usbPort = ?, networkIp = ?, networkPort = ?, isActive = ?, updatedAt = CURRENT_TIMESTAMP
         WHERE id = ?`, [destination, type, name, usbPort || null, networkIp || null, networkPort || null, isActive ? 1 : 0, id]);
    ...
    static async getByDestination(req, res) {
        try {
            ...
            const printer = await dbGet('SELECT * FROM printer_configs WHERE destination = ? AND isActive = 1 ORDER BY createdAt DESC LIMIT 1', [destination]);
            if (!printer) {
    ...
    static async testPrint(req, res) {
        try {
            ...
            // Test d'impression
            const success = await thermalPrintService_1.ThermalPrintService.testPrint(printer);
            if (success) {
                res.json({
                    success: true,
