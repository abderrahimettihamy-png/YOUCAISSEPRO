...
router.post('/order', auth_1.authenticate, (0, auth_1.authorize)(types_1.UserRole.SERVEUR, types_1.UserRole.CAISSIER), async (req, res) => {
    try {
    ...
    `, [orderId]);
        ...
        const destinations = new Set(itemsWithDefaults.map((item) => item.categoryType).filter((d) => d));
        let printResults = {
            printed: [],
            failed: []
        };
        ...
        if (destinations.size > 0) {
            for (const destination of destinations) {
                try {
                    ...
                    const destinationItems = itemsWithDefaults.filter((item) => item.categoryType === destination);
                    // Chercher une imprimante configurée pour cette destination
                    const printerDestination = destination === 'boissons' ? 'BAR' : 'CUISINE';
                    let printer;
                    try {
                        printer = await (0, database_1.dbGet)(`
...
router.get('/order/:id/tickets', auth_1.authenticate, (0, auth_1.authorize)(types_1.UserRole.SERVEUR, types_1.UserRole.CAISSIER), async (req, res) => {
    try {
        const { id } = req.params;
