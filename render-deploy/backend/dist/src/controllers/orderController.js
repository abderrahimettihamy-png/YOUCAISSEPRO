...
class OrderController {
    static async create(req, res) {
        try {
            ...
            const result = await (0, database_1.dbRun)("INSERT INTO orders (ticketNumber, serveurId, createdById, total, clientName, mealTime, notes, status) VALUES (?, ?, ?, ?, ?, ?, ?, 'en_attente')", [ticketNumber, serveurId, serveurId, total, clientName || null, mealTime, notes || null]);
            const orderId = result.lastID;
            for (const item of items) {
                const itemTotal = item.price * item.quantity;
                await (0, database_1.dbRun)('INSERT INTO order_items (orderId, productId, productName, quantity, price, total, addedById) VALUES (?, ?, ?, ?, ?, ?, ?)', [orderId, item.productId || null, item.productName, item.quantity, item.price, itemTotal, serveurId]);
                // Mettre à jour le stock si productId existe
                if (item.productId) {
                    await (0, database_1.dbRun)('UPDATE products SET stock = stock - ? WHERE id = ?', [item.quantity, item.productId]);
                }
            }
            res.status(201).json({
    ...
    static async getAll(req, res) {
      ...
      `);
            ...
            const ordersWithItems = await Promise.all(orders.map(async (order) => {
                const items = await (0, database_1.dbAll)(`
            SELECT oi.*, 
    ...
    static async addItems(req, res) {
        try {
            ...
            const order = await (0, database_1.dbGet)('SELECT * FROM orders WHERE id = ? AND status = ?', [id, 'en_attente']);
            if (!order) {
            ...
            for (const item of items) {
                const itemTotal = item.price * item.quantity;
                await (0, database_1.dbRun)('INSERT INTO order_items (orderId, productId, productName, quantity, price, total, addedById) VALUES (?, ?, ?, ?, ?, ?, ?)', [id, item.productId || null, item.productName, item.quantity, item.price, itemTotal, addedById]);
                // Mettre à jour le stock si productId existe
                if (item.productId) {
                    await (0, database_1.dbRun)('UPDATE products SET stock = stock - ? WHERE id = ?', [item.quantity, item.productId]);
                }
            ...
            await (0, database_1.dbRun)('UPDATE orders SET total = ?, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [newTotal, id]);
            res.json({
    ...
    static async update(req, res) {
        try {
            ...
            const order = await (0, database_1.dbGet)('SELECT status FROM orders WHERE id = ?', [id]);
            if (!order) {
            ...
            if (status) {
                ...
                params.push(id);
                await (0, database_1.dbRun)(updateQuery, params);
            }
            if (items && Array.isArray(items)) {
                // Supprimer les anciens articles
                await (0, database_1.dbRun)('DELETE FROM order_items WHERE orderId = ?', [id]);
                // Recalculer le total
                const total = items.reduce((sum, item) => {
                    return sum + (item.price * item.quantity);
                }, 0);
                // Mettre à jour le total
                await (0, database_1.dbRun)('UPDATE orders SET total = ?, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [total, id]);
                // Insérer les nouveaux articles
                for (const item of items) {
                    const itemTotal = item.price * item.quantity;
                    await (0, database_1.dbRun)('INSERT INTO order_items (orderId, productName, quantity, price, total) VALUES (?, ?, ?, ?, ?)', [id, item.productName, item.quantity, item.price, itemTotal]);
                }
            }
    ...
    static async delete(req, res) {
        try {
            ...
            const result = await (0, database_1.dbRun)('DELETE FROM orders WHERE id = ?', [id]);
            if (result.changes === 0) {
                return res.status(404).json({ error: 'Commande non trouvée' });
            }
    ...
    static async getDailySales(req, res) {
      ...
      `;
            if (date) {
                ...
                const result = await (0, database_1.dbGet)(query + ' GROUP BY DATE(createdAt)', [date]);
                return res.json(result || { date, totalSales: 0, orderCount: 0 });
            }
            query += ` AND DATE(createdAt) = DATE('now') GROUP BY DATE(createdAt)`;
            const result = await (0, database_1.dbGet)(query);
    ...
    static async getZReport(req, res) {
      ...
      `, [targetDate]);
            ...
            const paymentSummary = await (0, database_1.dbAll)(`
        SELECT 
          paymentMethod,
          COUNT(*) as count,
      ...
      `, [targetDate]);
            ...
            const itemsSummary = {};
            const drinksDetails = {}; // Détails des boissons
            items.forEach((item) => {
    ...
    static async searchByTicket(req, res) {
        try {
    ...
    static async clearSystem(req, res) {
        try {
      ...
      `);
            ...
            const result = await (0, database_1.dbRun)("DELETE FROM orders WHERE status IN ('payee', 'annulee')");
            res.json({
