...
exports.CashierPrinterController = void 0;
const database_1 = require("../config/database");
const net_1 = __importDefault(require("net"));
class CashierPrinterController {
    // Récupérer mes imprimantes
    static async getMyPrinters(req, res) {
        try {
            const userId = req.user?.id;
            const printers = await (0, database_1.dbAll)('SELECT * FROM cashier_printers WHERE caissierId = ? ORDER BY destination', [userId]);
            res.json(printers);
        }
        catch (error) {
    ...
    static async savePrinter(req, res) {
        try {
            ...
            const existing = await (0, database_1.dbGet)('SELECT id FROM cashier_printers WHERE caissierId = ? AND destination = ?', [userId, destination]);
            if (existing) {
                // Mise à jour
                await (0, database_1.dbRun)(`UPDATE cashier_printers 
           SET printerType = ?, printerName = ?, connectionInfo = ?, isActive = 1, updatedAt = CURRENT_TIMESTAMP
           WHERE caissierId = ? AND destination = ?`, [printerType, printerName, connectionInfo, userId, destination]);
            }
            else {
                // Création
                await (0, database_1.dbRun)(`INSERT INTO cashier_printers (caissierId, destination, printerType, printerName, connectionInfo, isActive)
           VALUES (?, ?, ?, ?, ?, 1)`, [userId, destination, printerType, printerName, connectionInfo]);
            }
            const updatedPrinters = await (0, database_1.dbAll)('SELECT * FROM cashier_printers WHERE caissierId = ? ORDER BY destination', [userId]);
    ...
    static async deletePrinter(req, res) {
        try {
            const userId = req.user?.id;
            const { id } = req.params;
            const printer = await (0, database_1.dbGet)('SELECT * FROM cashier_printers WHERE id = ? AND caissierId = ?', [id, userId]);
            if (!printer) {
                return res.status(404).json({ error: 'Imprimante non trouvée' });
            }
            await (0, database_1.dbRun)('DELETE FROM cashier_printers WHERE id = ?', [id]);
    ...
    static async printTicket(req, res) {
        try {
            ...
            const customization = await (0, database_1.dbGet)('SELECT * FROM ticket_customization WHERE caissierId = ?', [userId]);
            // Récupérer la commande avec détails
            const order = await (0, database_1.dbGet)(`
        SELECT 
          o.*,
          u.prenom, u.nom,
      ...
      `, [orderId]);
            ...
            const barItems = items.filter((item) => item.categoryId >= 1 && item.categoryId <= 4);
            const cuisineItems = items.filter((item) => item.categoryId >= 5);
            // Récupérer les imprimantes du caissier
            const printers = await (0, database_1.dbAll)('SELECT * FROM cashier_printers WHERE caissierId = ? AND isActive = 1', [userId]);
            const results = {
                printed: [],
                errors: []
            };
    ...
    static async saveCustomization(req, res) {
        try {
            const userId = req.user?.id;
            const { companyName, headerText, footerText, logoUrl, showDate, showTime, showServerName, fontSize, paperWidth } = req.body;
            // Vérifier si existe déjà
            const existing = await (0, database_1.dbGet)('SELECT id FROM ticket_customization WHERE caissierId = ?', [userId]);
            if (existing) {
                // Mise à jour
                await (0, database_1.dbRun)(`UPDATE ticket_customization 
           ...
           WHERE caissierId = ?`, [companyName, headerText, footerText, logoUrl, showDate, showTime,
                    showServerName, fontSize, paperWidth, userId]);
            }
            else {
                // Création
                await (0, database_1.dbRun)(`INSERT INTO ticket_customization 
           (caissierId, companyName, headerText, footerText, logoUrl, showDate, 
            showTime, showServerName, fontSize, paperWidth)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [userId, companyName, headerText, footerText, logoUrl, showDate,
                    showTime, showServerName, fontSize, paperWidth]);
            }
            const updated = await (0, database_1.dbGet)('SELECT * FROM ticket_customization WHERE caissierId = ?', [userId]);
...
exports.CashierPrinterController = CashierPrinterController;
// Générer ticket client ESC/POS
function generateClientTicketESCPOS(order, items, customization) {
    const commands = [];
    // Initialiser
...
function generateKitchenTicketESCPOS(order, items, destination) {
    const commands = [];
    // Initialiser
    commands.push(0x1B, 0x40);
    // Centrer + Gras
...
//# sourceMappingURL=cashierPrinterController.js.map