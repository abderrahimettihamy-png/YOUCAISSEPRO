...
const thermalPrintService_1 = require("../utils/thermalPrintService");
...
class PrintController {
    ...
    static async printOrder(req, res) {
        try {
            ...
            const order = await dbGet('SELECT o.*, u.nom, u.prenom FROM orders o LEFT JOIN users u ON o.serveurId = u.id WHERE o.id = ?', [orderId]);
            if (!order) {
      ...
      `, [orderId]);
            ...
            if (barItems.length > 0) {
                const barPrinter = await dbGet('SELECT * FROM printer_configs WHERE destination = ? AND isActive = 1 LIMIT 1', ['BAR']);
                if (barPrinter) {
                    // Impression réelle sur imprimante thermique
                    try {
                        const success = await thermalPrintService_1.ThermalPrintService.printTicket(barPrinter, {
            ...
            if (cuisineItems.length > 0) {
                const cuisinePrinter = await dbGet('SELECT * FROM printer_configs WHERE destination = ? AND isActive = 1 LIMIT 1', ['CUISINE']);
                if (cuisinePrinter) {
                    // Impression réelle sur imprimante thermique
                    try {
                        const success = await thermalPrintService_1.ThermalPrintService.printTicket(cuisinePrinter, {
                            ticketNumber: order.ticketNumber,
                            clientName: order.clientName,
                            mealTime: order.mealTime,
                            notes: order.notes,
    ...
    static async getTicketsByDestination(req, res) {
        try {
            const { orderId } = req.params;
            // Récupérer la commande
            const order = await dbGet(`SELECT o.*, u.nom, u.prenom 
         FROM orders o 
